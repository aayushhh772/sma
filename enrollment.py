...
class EnrollmentWindow(QWidget):
    ...
    def setup_ui(self):
        ...
        footer = QLabel("SOS Hermann Gmeiner School Gandaki Facial Enrollment System")
    ...
    def create_header(self):
        ...
        subtitle.setStyleSheet("color: #D9E8F5; font-size: 13px;")
        ...
        self.btn_dashboard = QPushButton("Back to Dashboard")
        self.btn_dashboard.setMinimumSize(160, 36)
        ...
        self.btn_dashboard.clicked.connect(self.go_back)
    ...
    def create_student_panel(self):
        ...
        progress_title = QLabel(f"FACIAL SAMPLES ({REQUIRED_SAMPLES} REQUIRED)")
        ...
        self.status = QLabel("Enter student information and start the camera.")
    ...
    def go_back(self):
        """Stops the active camera stream and returns to the dashboard context."""
        self.stop_camera()
    ...
    def initialize_face_system(self):
        ...
        try:
            ...
            self.status.setText("Face recognition model failed to load.")
            return False
    ...
    def validate_student_information(self):
        ...
        if not name:
            QMessageBox.warning(self, "Missing Name", "Enter the student's name.")
            return None
        if not student_id:
            QMessageBox.warning(self, "Missing Student ID", "Enter a student ID.")
            return None
        if not class_number:
            QMessageBox.warning(self, "Missing Class", "Enter the student's class.")
            return None
        if not section:
            QMessageBox.warning(self, "Missing Section", "Enter the student's section.")
            return None
    ...
    def start_camera(self):
        ...
        if not self.camera.isOpened():
            QMessageBox.critical(self, "Camera Error", "The camera could not be opened.")
            self.camera.release()
            self.camera = None
            return
        ...
        self.status.setText("Camera ready. Position one face clearly in the frame.")
        self.timer.start(30)
    ...
    def detect_faces(self, frame):
        ...
        if original_width > RECOGNITION_FRAME_WIDTH:
            ...
            recognition_frame = cv2.resize(
                frame, (RECOGNITION_FRAME_WIDTH, resized_height)
            )
        else:
        ...
        try:
            faces = self.face_app.get(recognition_frame)
        except Exception:
            return [], None
    ...
    def update_camera(self):
        ...
        if len(faces) == 1:
            ...
            if self.current_face is not None:
                cv2.rectangle(display, (x1, y1), (x2, y2), (46, 204, 113), 3)
                cv2.putText(
                    display,
                    "FACE READY",
                    (x1, max(30, y1 - 10)),
                ...
                cv2.rectangle(display, (x1, y1), (x2, y2), (52, 152, 219), 3)
                cv2.putText(
                    display,
                    "MOVE CLOSER",
                    (x1, max(30, y1 - 10)),
    ...
    def capture_face_sample(self):
        ...
        embedding = np.asarray(self.current_face.embedding, dtype=np.float32).flatten()
        if embedding.size == 0 or not np.all(np.isfinite(embedding)):
            self.status.setText("Invalid facial embedding detected.")
            return
        ...
        self.captured_embeddings.append(embedding.tolist())
        sample_count = len(self.captured_embeddings)
        self.progress.setValue(sample_count)
        ...
        if sample_count < REQUIRED_SAMPLES:
            ...
            self.status.setText("All facial samples captured. You can now save enrollment.")
    ...
    def save_enrollment(self):
        ...
        try:
            existing_student = get_student(student["student_id"])
        except Exception as error:
            QMessageBox.critical(
                self,
                "Database Error",
        ...
        try:
            ...
            self.status.setText("Enrollment could not be saved.")
            return
    ...
    def clear_enrollment_form(self):
        ...
        self.status.setText("Enter student information and start the camera.")
    ...
    def stop_camera(self):
        ...
        self.camera_view.setText("Camera is not active")
